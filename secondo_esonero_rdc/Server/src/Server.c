...
int main(int argc, char *argv[]) {
	...
	char * server_ip = DEFAULT_ADDRESS;
	int port = PROTOPORT;
	printf("%s %s:%d\n", "Trying to start server on", server_ip, port);
	...
	server_address_struct.sin_addr.s_addr = inet_addr(server_ip);

	int socket_bind = bind(server_socket, (struct sockaddr*)&server_address_struct, server_address_size);
	...
	while (1){
		...
		do{
			...
			client_data_size = recvfrom(server_socket, client_data, ECHOMAX - 1, 0, (struct sockaddr*)&client_address, &client_address_size);
			...
			struct in_addr addr;
			addr.s_addr = inet_addr(inet_ntoa(client_address.sin_addr));
			remote_host = gethostbyaddr((char *)&addr, 4, AF_INET);
			char * canonical_name = remote_host->h_name;

			printf("%s%s%s%s%s%s\n", "Requested operation '", client_data, "' from client ", canonical_name, ", ip ", inet_ntoa(client_address.sin_addr));
			...
			if(strcmp(string_tokens[0], "=") == 0){
				...
				int data_send_size;
				data_send_size = sendto(server_socket, operation_result, string_len, 0, (struct sockaddr*)&client_address, client_address_size);
				if(data_send_size < 0){
					errorHandler("Something went wrong sending the packet");
				}
				printf("%s", "Operation sent to the client\n");
			}
		}while(continue_loop);
	}
